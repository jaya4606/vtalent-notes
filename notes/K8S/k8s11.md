...
kind: Deployment
metadata:
  name: deployment-nodeselectorfail
  labels: 
    purpose: learning
spec:
  selector:
  ...
  template:
    metadata:
      ...
      spec: 
        containers:
          - name: nginx-deployment-nodeselector-fail
            image: nginx:latest
            ports:
              - containerPort: 80
        nodeSelector:
          type: db # add a label to nodes
```
      ...
      spec: 
        containers:
          - name: nginx-deployment-nodename
            ...
            ports:
              - containerPort: 80
        nodeName: node1 # give nodes name
```
